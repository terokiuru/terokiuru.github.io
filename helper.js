...
// var content_string = '<div class="row"> <div class="col-md-3"> </div> <div class="col-md-6" > <p>%data%</p><input type="text" id="%id%"> % </div><div class="col-md-3"></div></div>';
var script_string = '<script> var slider%id3% = document.getElementById("%id4%"); var output%id5% = document.getElementById("demo%id6%"); output%id7%.innerHTML = slider%id8%.value; slider%id9%.oninput = function() {output%id10%.innerHTML = this.value;}</script>'
var content_string = '<div class="row"> <div class="col-md-3"> </div> <div class="col-md-6" > <p>%data%</p> <div class="slidecontainer"> <input type="range" min="0" max="100" value="50" class="slider" id="%id%"><p>Todennäköisyys: <span id="demo%id2%"></span> %</p></div> %script% </div><div class="col-md-3"></div></div> ';
var hr_string = '<div class="row"> <div class="col-md-3"> </div> <div class="col-md-6"> <hr> </div><div class="col-md-3"></div></div>';
...
function up_date() {
	...
	for (var i = 0; i < number_of_questions; i++) {
		slider_ids[i] = i;
		append_string = content_string + hr_string;
		id = questions[i].id;
		text = questions[i].text_fi;
		question_strings[i] = text;
		append_string = append_string.replace("%script%", script_string)
		...
		append_string = append_string.replace("%id3%", id);
		append_string = append_string.replace("%id4%", id);
		append_string = append_string.replace("%id5%", id);
		append_string = append_string.replace("%id6%", id);	
		append_string = append_string.replace("%id7%", id);
		append_string = append_string.replace("%id8%", id);
		append_string = append_string.replace("%id9%", id);
		append_string = append_string.replace("%id10%", id);						
